...
/* ------------------------------ Config -------------------------------- */
  ...
  #define  GUEST_PASSWORD  ""   // empty = guest/telemetry-only login disabled until explicitly set
#endif
...
// protects against spam/accidental loops regardless of who's asking.
  ...
  #define WEATHER_REPLY_COOLDOWN_MS  (60UL * 1000)
#endif
...
uint8_t SensorMesh::handleRequest(uint8_t perms, uint32_t sender_timestamp, uint8_t req_type, uint8_t* payload, size_t payload_len) {
  ...
  if (req_type == REQ_TYPE_GET_TELEMETRY_DATA) {  // allow all
    ...
    telemetry.reset();
    // On WS_CH_BATTERY (2), not the default TELEM_CHANNEL_SELF (1) -- that's
    // reserved for weather so it isn't mixed in behind battery.
    telemetry.addVoltage(WS_CH_BATTERY, (float)board.getBattMilliVolts() / 1000.0f);
...
uint8_t SensorMesh::handleLoginReq(const mesh::Identity& sender, const uint8_t* secret, uint32_t sender_timestamp, const uint8_t* data, bool is_flood) {
  ...
  uint8_t base_perms = 0, role_perms = PERM_ACL_GUEST;

  if (data[0] == 0) {   // blank password
    client = acl.getClient(sender.pub_key, PUB_KEY_SIZE);
    if (client == NULL) {
      // Unknown sender, no password offered -- auto-admit as guest (telemetry-read
      // only, no admin/config rights). Telemetry from this node is intentionally
      // public: no password or pre-whitelisting by pubkey required to query it.
      need_registration = true;
    }
  } else if (strcmp((char *) data, _prefs.password) == 0) {  // valid admin password
    base_perms = PERM_RECV_ALERTS_HI | PERM_RECV_ALERTS_LO;
    role_perms = PERM_ACL_ADMIN;
    need_registration = true;
  } else if (_prefs.guest_password[0] != 0 && strcmp((char *) data, _prefs.guest_password) == 0) {
    need_registration = true;  // role_perms already defaulted to PERM_ACL_GUEST above
  } else {
    #if MESH_DEBUG
  ...
  if (need_registration) {
    client = acl.putClient(sender, base_perms);  // add to contacts (if not already known)
...
void SensorMesh::onPeerDataRecv(mesh::Packet* packet, uint8_t type, int sender_idx, const uint8_t* secret, uint8_t* data, size_t len) {
  ...
  if (type == PAYLOAD_TYPE_REQ) {  // request (from a known contact)
  ...
  } else if (type == PAYLOAD_TYPE_TXT_MSG && len > 5) {   // a plain-text DM or a CLI command
    ...
    if (sender_timestamp > from->last_timestamp) {  // prevent replay attacks
      if (flags == TXT_TYPE_PLAIN) {
        ...
        if (!from->isAdmin()) {   // CLI commands (e.g. "channel join") stay admin-only
          MESH_DEBUG_PRINTLN("onPeerDataRecv: rejecting CLI command from non-admin");
        } else {
        from->last_timestamp = sender_timestamp;
        from->last_activity = getRTCClock()->getCurrentTime();
...
bool SensorMesh::handleIncomingMsg(ClientInfo& from, uint32_t timestamp, uint8_t* data, uint8_t flags, size_t len) {
  ...
  if (!weatherReplyAllowed()) {
    return false;  // same as an unrecognized command: no ack, no reply
  }

  char report[140];
  sensors.getWeatherReportText(report, sizeof(report));
  int report_len = strlen(report);

  uint8_t temp[5 + sizeof(report)];
  uint32_t reply_timestamp = getRTCClock()->getCurrentTimeUnique();
  memcpy(temp, &reply_timestamp, 4);
  temp[4] = 0;  // TXT_TYPE_PLAIN, attempt 0
  memcpy(&temp[5], report, report_len);

  auto pkt = createDatagram(PAYLOAD_TYPE_TXT_MSG, from.id, from.shared_secret, temp, 5 + report_len);
  if (pkt) {
    if (from.out_path_len == OUT_PATH_UNKNOWN) {
      sendFlood(pkt);
    } else {
      sendDirect(pkt, from.out_path, from.out_path_len);
    }
  }
  return true;  // handled -- caller will also send a delivery ack
}
...
int SensorMesh::searchChannelsByHash(const uint8_t* hash, mesh::GroupChannel dest[], int max_matches) {
  int n = 0;
  for (int i = 0; i < num_weather_channels && n < max_matches; i++) {
    if (weather_channels[i].channel.hash[0] == hash[0]) {
      dest[n++] = weather_channels[i].channel;
    }
  }
  return n;
}

bool SensorMesh::joinWeatherChannel(const char* name, const uint8_t* secret, int secret_len) {
  if (secret_len != 16 && secret_len != 32) return false;
  if (name[0] == 0 || strlen(name) >= sizeof(weather_channels[0].name)) return false;

  for (int i = 0; i < num_weather_channels; i++) {
    if (strcmp(weather_channels[i].name, name) == 0) return true;  // already joined
  }
  if (num_weather_channels >= MAX_WEATHER_CHANNELS) return false;

  auto dest = &weather_channels[num_weather_channels];
...
bool SensorMesh::leaveWeatherChannel(const char* name) {
  for (int i = 0; i < num_weather_channels; i++) {
    if (strcmp(weather_channels[i].name, name) == 0) {
...
// "channel join #name"             -- hashtag channel, key derived from name
// "channel join name <psk-b64>"    -- private channel, explicit 16/32-byte key
// "channel leave name"
// "channel list"
void SensorMesh::handleChannelCommand(char* args, char* reply) {
  while (*args == ' ') args++;

  if (memcmp(args, "list", 4) == 0) {
    if (num_weather_channels == 0) {
      strcpy(reply, "(no channels joined)");
    } else {
      char* p = reply;
      for (int i = 0; i < num_weather_channels; i++) {
        p += sprintf(p, "%s%s", i ? ", " : "", weather_channels[i].name);
      }
    }
  } else if (memcmp(args, "join ", 5) == 0) {
    char* name = &args[5];
    char* sp = strchr(name, ' ');
    uint8_t secret[32];
    int secret_len;

    if (sp != NULL) {   // explicit PSK given -- private channel
      *sp++ = 0;
      secret_len = decode_base64((unsigned char *) sp, strlen(sp), secret);
    } else if (name[0] == '#') {   // hashtag channel -- derive key from name
      secret_len = 16;
      mesh::Utils::sha256(secret, secret_len, (const uint8_t *) name, strlen(name));
    } else {
      strcpy(reply, "Err - need a #hashtag name, or 'join name <psk-b64>'");
      return;
    }
...
void SensorMesh::onGroupDataRecv(mesh::Packet* packet, uint8_t type, const mesh::GroupChannel& channel, uint8_t* data, size_t len) {
  if (type != PAYLOAD_TYPE_GRP_TXT) return;   // only free-text messages carry commands
  if (len < 5) return;

  uint8_t txt_type = data[4];
  if ((txt_type >> 2) != 0) return;   // not a plain-text message

  data[len] = 0;  // null-terminate (packet payload buffer has slack for this)
...
bool SensorMesh::weatherReplyAllowed() {
  unsigned long now = millis();
  if ((long)(now - _last_weather_reply_ms) < (long)WEATHER_REPLY_COOLDOWN_MS) return false;
  _last_weather_reply_ms = now;
  return true;
}

void SensorMesh::sendWeatherReport(const mesh::GroupChannel& channel) {
  char report[140];
  sensors.getWeatherReportText(report, sizeof(report));

  char msg[5 + sizeof(_prefs.node_name) + 2 + sizeof(report)];
  snprintf(msg, sizeof(msg), "%s: %s", _prefs.node_name, report);
  int msg_len = strlen(msg);

  // Mesh::createGroupDatagram() rejects data_len + 1 + (CIPHER_BLOCK_SIZE-1) >
  // MAX_PACKET_PAYLOAD -- clamp so a verbose report never gets silently dropped.
  const int max_msg_len = MAX_PACKET_PAYLOAD - 5 - (CIPHER_BLOCK_SIZE - 1);
  if (msg_len > max_msg_len) msg_len = max_msg_len;

  uint8_t temp[5 + max_msg_len];
      ...
      telemetry(MAX_PACKET_PAYLOAD - 4)
{
  ...
  StrHelper::strncpy(_prefs.guest_password, GUEST_PASSWORD, sizeof(_prefs.guest_password));
  ...
  _prefs.flood_advert_interval = 47;  // 47 hours, matches simple_repeater/simple_room_server default
